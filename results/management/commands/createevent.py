...
from results.models.competitions import CompetitionResultType, CompetitionType, CompetitionLevel, CompetitionLayout
...
def _create_categories(sport=None, verbosity=1):
    ...
    for c in classes:
        category = Category.objects.get_or_create(
            abbreviation=c[0], name=c[1], max_age=c[2], min_age=c[3], gender=c[4], sport=sport, division=None,
            historical=False)
        if verbosity > 1:
            print(category)
...
def _create_organizations(verbosity=1):
    ...
    organizations = [['Huippu', 'Huippu-urheilijat'],
                     ['KK', 'Kyläkisaajat'],
                     ['NK', 'Napakymppi ry'],
...
def _create_competition_types(sport=None, record_level=None, verbosity=1):
    ...
    competition_types = [['300KA', '300 m kivääri asentokilpailu'],
    ...
    for c in competition_types:
        competition_type = CompetitionType.objects.get_or_create(
            abbreviation=c[0], name=c[1], number_of_rounds=4, max_result=654, min_result=0, sport=sport,
            historical=False, layout=1, team=False)
        if record_level:
...
def _create_competition_result_types(verbosity=1):
    ...
    for c in CompetitionType.objects.all():
        if not CompetitionResultType.objects.filter(competition_type=c).count():
            CompetitionResultType.objects.get_or_create(competition_type=c, name='Partial', abbreviation='part',
                                                        max_result=100)
            CompetitionResultType.objects.get_or_create(competition_type=c, name='Final', abbreviation='fin',
                                                        max_result=100)

        if not CompetitionLayout.objects.filter(type=c.layout).count():
            CompetitionLayout.objects.get_or_create(type=c.layout, label='preliminary', name='nolimit',
                                                    block=0, row=1, col=0, order=1, )
            CompetitionLayout.objects.get_or_create(type=c.layout, label='#', name='position',
                                                    block=1, row=1, col=1, order=2)
            CompetitionLayout.objects.get_or_create(type=c.layout, label='Athlete', name='athlete',
                                                    block=1, row=1, col=2, order=3)
            CompetitionLayout.objects.get_or_create(type=c.layout, label='Club', name='organization',
                                                    block=1, row=1, col=3, order=4)
            CompetitionLayout.objects.get_or_create(type=c.layout, label='S1', name='part-1',
                                                    block=1, row=1, col=4, order=5)
            CompetitionLayout.objects.get_or_create(type=c.layout, label='S2', name='part-2',
                                                    block=1, row=1, col=5, order=6)
            CompetitionLayout.objects.get_or_create(type=c.layout, label='Result', name='result',
                                                    block=1, row=1, col=6, order=7)
            CompetitionLayout.objects.get_or_create(type=c.layout, label='F1', name='fin-1',
                                                    block=1, row=2, col=4, order=8)
            CompetitionLayout.objects.get_or_create(type=c.layout, label='F1', name='fin-2',
                                                    block=1, row=2, col=5, order=9)
            CompetitionLayout.objects.get_or_create(type=c.layout, label='Sum', name='fsum',
                                                    block=1, row=2, col=6, order=10)
...
def _create_competition_levels(record_level=None, verbosity=1):
    ...
    competition_levels = [['Olympics', 'Olympics', True],
    ...
    for c in competition_levels:
        competition_level = CompetitionLevel.objects.get_or_create(
            abbreviation=c[0], name=c[1], historical=False)
...
def create_base_data(verbosity=1):
    ...
    sport = Sport.objects.get_or_create(
        abbreviation="Kivääri", name="Kivääri", historical=False)[0]
...
def _create_athletes(number, verbosity=1):
    ...
    for x in range(0, number):
        ...
        athlete = AthleteFactory.create(gender=gender, first_name=first_name,
...
def _get_category(athlete, competition):
    ...
    categories = Category.objects.filter(
        ...
        sport=competition.type.sport, team=False)
...
def _create_competition(event, competition_type, verbosity=1):
    ...
    competition = CompetitionFactory.create(
        event=event, date_start=competition_date, date_end=competition_date, name=event.name,
        organization=event.organization, type=competition_type, level=competition_level,
        location=event.location, layout=competition_type.layout)
    if verbosity:
...
def _get_decimals(competition):
    if competition.type.sport.abbreviation == "Practical":
        decimals = 2
    elif ((competition.type.max_result and competition.type.max_result % 10 == 0)
...
def _create_results(competition, index, number, verbosity=1):
    ...
    competition.
    """
    layouts = CompetitionLayout.objects.filter(type=competition.type.layout,
                                               name__icontains='-').order_by('order')
    ...
    for i in range(1, number):
        ...
        result = Result.objects.create(
            competition=competition, athlete=athletes[i], organization=athletes[i].organization,
            category=category, elimination_category=category, decimals=decimals, result=0)
        result_sum = 0
            ...
            if 'f' not in layout.name and 'sum' not in layout.name:
                ...
                if temp_decimals:
                    value = decimal.Decimal(
                        random.randint((max_result - round((max_result / level * 10))) * 10,
                                       max_result * 10)) / 10
                else:
                    value = decimal.Decimal(random.randint((max_result - round((max_result / level * 10))),
                                                           max_result))
                ResultPartial.objects.create(result=result, type=pr_type, order=layout.name.split('-')[1], value=value,
                                             decimals=temp_decimals)
                part_sum = part_sum + value
                result_sum = result_sum + value
            elif 'f' not in layout.name and 'sum' in layout.name:
                pr_type = partial_result_types.filter(abbreviation=layout.name.split('-')[0])[0]
                ResultPartial.objects.create(result=result, type=pr_type, order=layout.name.split('-')[1],
                                             value=part_sum, decimals=temp_decimals)
                part_sum = 0
        if layouts.count() == 0:
...
def _create_finals_partial_results(position, category_result, layouts, partial_result_types, competition_type):
    ...
    for p in layouts:
        if 'fin' in p.name and '-' in p.name:
            ...
            if ((competition_type in [1, 2, 20, 22] and position < 9 and order + position < 12) or
                    (competition_type == 5 and position < 9 and (order < 9 or order + position < 16)) or
                    (competition_type in [21, 27] and position < 7 and (order < 5 or position + order < 11)) or
                    (competition_type == 23 and position < 9 and (order < 5 or position + order < 13))):
                result = decimal.Decimal(random.randint(
                    (max_result * 10) - round((max_result / 5 * position)),
                    (max_result * 10) - round((max_result / 10 * position)))) / 10
                if not decimals:
...
def _create_finals(competition):
    ...
    """
    layouts = CompetitionLayout.objects.filter(type=competition.type.layout,
                                               name__icontains='-').filter(name__icontains='f')
    partial_result_types = CompetitionResultType.objects.filter(competition_type=competition.type)
    layouts_count = layouts.count()
    categories = Result.objects.filter(competition=competition).order_by('category').values_list(
        ...
        for category_result in category_results:
            if competition.type.sport.abbreviation == "Practical":
                if position == 1:
                    ...
                    category_result.result = decimal.Decimal(round(100 / (random.randint(position + 10,
                                                                                         position * 2 + 10) / 10), 2))
            ...
            if layouts_count > 0 and category_size > 3 and position < 9:
                _create_finals_partial_results(position, category_result, layouts, partial_result_types,
                                               competition.type.id)
            position += 1
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('-b', action='store_true', dest='base_data',
                            help='Create base data')
    ...
    def handle(self, *args, **options):
        ...
        for sport in Sport.objects.all():
            event = EventFactory.create(organization=Organization.objects.order_by('?').first(),
