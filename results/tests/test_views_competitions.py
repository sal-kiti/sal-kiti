...
from results.tests.factories.competitions import (
    CompetitionFactory,
    CompetitionLayoutFactory,
    CompetitionLevelFactory,
...
class CompetitionTypeTestCase(ResultsTestCase):
    def setUp(self):
        ...
        self.data = {
            "name": self.object.name,
            "abbreviation": self.object.abbreviation,
            "number_of_rounds": self.object.number_of_rounds,
            "max_result": self.object.max_result,
...
class CompetitionResultTypeTestCase(ResultsTestCase):
    def setUp(self):
        ...
        self.data = {
            "competition_type": self.object.competition_type.id,
            "name": self.object.name,
            "abbreviation": self.object.abbreviation,
            "max_result": self.object.max_result,
            "min_result": self.object.min_result,
...
class CompetitionTestCase(ResultsTestCase):
    def setUp(self):
        ...
        self.data = {
            ...
            "type": self.object.type.pk,
            "level": self.object.level.pk,
    ...
    def test_competition_has_results(self):
        request = self.factory.get(self.url + "1/")
        view = self.viewset.as_view(actions={"get": "retrieve"})
        response = view(request, pk=self.object.pk)
        self.assertFalse(response.data["has_results"])
        result = ResultFactory.create(competition=self.object, public=False)
        response = view(request, pk=self.object.pk)
        self.assertFalse(response.data["has_results"])
        result.public = True
        result.save()
