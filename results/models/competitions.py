...
class CompetitionLayout(LogChangesMixing, models.Model):
    ...
    SIZE_CHOICES = [("sm", "Small"), ("md", "Medium"), ("lg", "Large"), ("xl", "Extra large")]
...
class CompetitionLevel(LogChangesMixing, models.Model):
    ...
    require_approval = models.BooleanField(
        default=False, verbose_name=_("Require approved competition to add results")
    )
...
class CompetitionType(LogChangesMixing, models.Model):
    ...
    max_result = models.DecimalField(
        blank=True, null=True, max_digits=12, decimal_places=3, verbose_name=_("Maximum result")
    )
    min_result = models.DecimalField(
        blank=True, null=True, max_digits=12, decimal_places=3, verbose_name=_("Minimum result")
    )
    order = models.SmallIntegerField(default=0, verbose_name=_("Order"))
    requirements = models.CharField(blank=True, max_length=100, verbose_name=_("Requirements"))
...
class Competition(LogChangesMixing, models.Model):
    ...
    def has_object_write_permission(self, request):
        if (
            ...
            or (
                self.organization.is_manager(request.user)
    ...
    def has_object_update_permission(self, request):
        if (
            ...
            or (self.organization.is_manager(request.user) and not self.locked and not self.event.locked)
        ):
            return True
        return False
